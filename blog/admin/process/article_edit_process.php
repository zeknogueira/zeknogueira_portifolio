...
$sql = "UPDATE articles SET article_name = ?, article_description = ?, article_content = ?, article_category = ? WHERE article_id = ?";

$stmt = $conection_db->prepare($sql);
if ($stmt) {
    $stmt->bind_param("ssssi", $article_name, $article_description, $article_content, $article_category, $article_id_send);
